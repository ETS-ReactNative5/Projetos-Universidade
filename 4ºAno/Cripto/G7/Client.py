...
class Client:
    ...
    def process(self, msg = b""):
        ...
        # Se receber a 1ª mensagem, trata-se da assinatura e da chave pública de DH do
        #   servidor
        elif self.msg_count == 0:
            print('> Server [{}] sent its signature and public key.'.format(self.msg_count))
            
            # Os primeiros 256 bytes estão reservados para a assinatura do servidor
            sv_signature = msg[:256]

            # Chave pública de DH do servidor
            sv_key_public_dh_bytes = msg[256:]
            ...
            # Leitura da chave pública de RSA do servidor armazenada
            f = open('sv_key_public_rsa.txt', 'rb')
            sv_key_public_rsa_bytes = f.read()
            f.close()
            
            # Validação da chave pública de RSA do servidor
            try:
                sv_key_public_rsa = load_pem_public_key (
                    sv_key_public_rsa_bytes,
                    backend = default_backend()
                )

            except:
                print('» Invalid RSA key from Server!')
            ...
            # Leitura da chave privada de RSA do cliente armazenada
            f = open('cl_key_private_rsa.txt', 'rb')
            cl_key_private_rsa_bytes = f.read()
            f.close()

            # Validação da chave privada de RSA do cliente
            try:
        ...
        if self.msg_count == 0:
            return cl_signature + msg_tag + msg_enc
