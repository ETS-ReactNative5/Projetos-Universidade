...
class Connection(object):
    ...
    def process(self, msg):
        ...
        # Se não receber mensagem vazia
            ...
            # Se for a 1ª mensagem, trata-se da chave pública do cliente
            if self.msg_count == 0:
                print('> Client #{} [{}] sent its public key.'.format(self.id, self.msg_count))

                # Chave pública do cliente
                cl_key_public = load_pem_public_key (
                    msg,
                    backend = default_backend()
                )

                # Chave partilhada (gerada a partir da chave privada do servidor
                #   e da pública do cliente)
                key_shared = sv_key_private.exchange(cl_key_public)
                ...
                print('» Sending my public key #{}...'.format(self.id))

                # Envio da chave pública do servidor
                return sv_key_public
                ...
                if not mac_check:
                    ...
                    msg_dec = decrypt(msg_enc, self.key_enc, self.msg_count - 1)
