...
public class MasterClient {
    private static final int PCKT_SIZE = 8820, SV_PORT = 5000, BC_PORT = 6000;
    ...
    private static boolean playing = false, loading = true;
        ...
        return new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    for (; (currentSN < maxSN && currentSN < musicLength && playing); currentSN ++) {
                    ...
                    if (currentSN < musicLength && playing) {
                        printAndBar("> Loading...");
                        byte[] bytes = {(byte) 0, (byte) (currentSN >> 8), (byte) (currentSN)};
                        printAndBar("> TCP sending to Gateway: "
                                + String.format("%02X %02X%02X", bytes[0], bytes[1], bytes[2]) + "\n");
                        socket.getOutputStream().write(bytes);                        
                        loading = true;
                    }

                    else if (playingId > 1 && playing) {
                        printAndBar("> Auto-skipping to next music...");
                        if (playingId - 2 + 1 == musicList.size())
                            playingId = 2;

                        else
                            playingId ++;

                        byte[] bytes = {(byte) playingId};

                        printAndBar("> TCP sending to Gateway: " + String.format("%02X", bytes[0]) + "\n");
                        socket.getOutputStream().write(bytes);                        
                        playing = false;
                    }
    ...
    public static void main(String[] args) throws UnknownHostException, IOException {
        ...
        printAndBar("\n--- [ Main menu ] ---");
        printAndBar("[0] Pause/resume");
        printAndBar("[1] Replay");
        printAndBar("---------------------");

        for (int i = 0; i < musicList.size(); i++) {
            Music music = musicList.get(i);
            printAndBar("[" + (i + 2) + "] " + music.getName() + " (" + timeFormat(music.getDuration() * 1000) + ")");
        }
        ...
        return new Runnable() {
            ...
            public void run() {
                ...
                while ((nLine = scanner.nextLine()) != null) {
                    try {
                        int request = Integer.parseInt(nLine);

                        if (request > 0) { // escolha de música ou reinício
                            if (request > 1) playingId = request; // escolha de música
                        ...
                        try {
                            ...
                            if (!storedSNs.contains(pcktCount) && pcktCount < maxSN + 16) {
                                ...
                                if (pcktCount > maxSN) {
                                    ...
                                    if (loading && maxSN > currentSN + 100) {
                                        printAndBar("> Loaded");
                                        byte[] bytes = {(byte) 0, (byte) (currentSN >> 8), (byte) (currentSN)};
                                        printAndBar("> TCP sending to Gateway: "
                                                + String.format("%02X %02X%02X", bytes[0], bytes[1], bytes[2]) + "\n");
